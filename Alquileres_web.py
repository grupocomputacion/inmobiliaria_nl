...
with st.sidebar:
    ...
    st.divider()
    # Aseguramos que el radio button maneje las opciones correctamente
...
# ==========================================
# 4. LÓGICA DE SECCIONES (USANDO IF INDEPENDIENTES)
# ==========================================
    ...
    if not inm.empty and not inq.empty:
        with st.form("f_contrato", clear_on_submit=True):
            ...
            if st.form_submit_button("GRABAR CONTRATO"):
                ...
                conn.commit(); st.success("Guardado exitoso."); st.rerun()
    else: st.warning("Debe cargar Inquilinos y Unidades en Maestros primero.")
    conn.close()
    ...
    if not deudas.empty:
        for _, r in deudas.iterrows():
            with st.expander(f"💰 {r['nombre']} - {r['tipo']} ({r['concepto']} {r['mes_anio']})"):
                ...
                c1, c2 = st.columns(2)
                if c1.button("Cobrar Ahora", key=f"btn_{r['id']}"):
...
if menu == "🚨 4. Morosos":
    ...
    conn = conectar()
    df_m = pd.read_sql_query("""SELECT inq.nombre as Inquilino, i.tipo as Unidad, d.concepto, d.mes_anio as Mes, (d.monto_debe - d.monto_pago) as Saldo FROM deudas d JOIN contratos c ON d.id_contrato=c.id JOIN inmuebles i ON c.id_inmueble=i.id JOIN inquilinos inq ON c.id_inquilino=inq.id WHERE d.pagado=0""", conn)
    if not df_m.empty:
        df_m['Saldo'] = df_m['Saldo'].apply(fmt_moneda)
        st.dataframe(df_m, use_container_width=True, hide_index=True)
    else: st.success("Sin morosos registrados.")
    conn.close()
...
if menu == "⚙️ 6. Maestros":
    ...
    with t1: # INQUILINOS
        ...
        with c2:
            if not inqs_df.empty:
                id_del = st.selectbox("Borrar Inquilino", inqs_df['id'].tolist(), format_func=lambda x: inqs_df[inqs_df['id']==x]['nombre'].values[0])
                if st.button("🗑️ Borrar Inquilino"): con.execute("DELETE FROM inquilinos WHERE id=?", (id_del,)); con.commit(); st.rerun()
        con.close()
    ...
    with t3: # UNIDADES
        ...
        with c2:
            if not unid_df.empty:
                id_ud = st.selectbox("Borrar Unidad", unid_df['id'].tolist(), format_func=lambda x: f"{unid_df[unid_df['id']==x]['b_nom'].values[0]} - {unid_df[unid_df['id']==x]['tipo'].values[0]}")
    ...
    with t4: # CONTRATOS
        ...
        if not df_c.empty:
            ...
            if st.button("🗑️ ELIMINAR CONTRATO"): con.execute("DELETE FROM deudas WHERE id_contrato=?", (idc,)); con.execute("DELETE FROM contratos WHERE id=?", (idc,)); con.commit(); st.rerun()
        con.close()

    with t5: # PROCESOS
        st.subheader("Generación Masiva de Cuotas")
        ...
        if st.button("Generar Alquileres"):
            ...
            con.commit(); con.close(); st.success("Generado con éxito.")
