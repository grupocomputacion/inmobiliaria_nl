...
# ==========================================
    ...
    else: st.warning("Debe cargar Inquilinos y Unidades en la sección de Maestros.")
    conn.close()

# ==========================================
# 💰 3. COBRANZAS (WhatsApp integrado)
# ==========================================
elif menu == "💰 3. Cobranzas":
    st.header("Gestión de Cobros")
    conn = conectar()
    deudas = pd.read_sql_query("""
        SELECT d.id, inq.nombre, i.tipo, d.concepto, d.mes_anio, d.monto_debe, d.monto_pago, inq.celular
    ...
    if deudas.empty: st.info("No hay cuotas pendientes.")
    for _, r in deudas.iterrows():
        with st.expander(f"📌 {r['nombre']} - {r['tipo']} ({r['mes_anio']})"):
            ...
            c1, c2 = st.columns(2)
            if c1.button("Confirmar Pago", key=f"btn_{r['id']}"):
                ...
                conn.commit()
                # Link de WhatsApp
                msg = urllib.parse.quote(f"✅ NL PROPIEDADES - RECIBO\nUnidad: {r['tipo']}\nAbonado: {fmt_moneda(limpiar_monto(pago))}\nSaldo Pendiente: {fmt_moneda(saldo - limpiar_monto(pago))}")
                st.markdown(f'<a href="https://wa.me/{r["celular"]}?text={msg}" target="_blank">📲 Enviar Comprobante por WhatsApp</a>', unsafe_allow_html=True)
                st.rerun()
            if c2.button("🗑️ Eliminar Cuota", key=f"del_{r['id']}"):
...
# ==========================================
    ...
    df_m = pd.read_sql_query("""
        SELECT inq.nombre as Inquilino, i.tipo as Unidad, d.concepto as Concepto, d.mes_anio as Periodo,
               (d.monto_debe - d.monto_pago) as Saldo FROM deudas d 
    ...
    if not df_m.empty:
        ...
        st.table(df_m)
    else: st.success("No hay deudores registrados.")
...
# ==========================================
    ...
    with t1: # INQUILINOS CON NUEVOS CAMPOS Y PREVISUALIZACIÓN
        con = conectar()
        ...
        with col_i1:
            ...
            if modo == "Editar" and not inqs_df.empty:
                sel_i = st.selectbox("Seleccionar:", inqs_df['id'].tolist(), format_func=lambda x: inqs_df[inqs_df['id']==x]['nombre'].values[0])
            ...
            with st.form("f_inquilino"):
                ...
                f_dir = st.text_input("Dirección Particular", value=v_dir)
                st.divider()
                c_a, c_b, c_c = st.columns(3)
                f_pro = c_a.text_input("Procedencia", value=v_pro)
                f_gru = c_b.text_input("Grupo/Clasificación", value=v_gru)
                f_eme = c_c.text_input("Contacto Emergencia", value=v_eme)
        ...
        with col_i2:
            ...
            if not inqs_df.empty:
                id_del = st.selectbox("Elegir para eliminar", inqs_df['id'].tolist(), format_func=lambda x: inqs_df[inqs_df['id']==x]['nombre'].values[0], key="del_inq")
                if st.button("ELIMINAR DEFINITIVAMENTE"):
                    con.execute("DELETE FROM inquilinos WHERE id=?", (id_del,))
                    con.commit(); st.rerun()

        st.write("### 📋 Ubicación Actual de Inquilinos")
    ...
    with t2: # BLOQUES
        ...
        nb = st.text_input("Nombre del Bloque (Ej: Torre A)")
        ...
        with c_u2:
            ...
            if not inms.empty:
                ...
                with st.form("f_u_edit"):
                    e1 = st.text_input("Alquiler", value=str(int(u['precio_alquiler'])))
                    e2 = st.text_input("Contrato", value=str(int(u['costo_contrato'])))
                    e3 = st.text_input("Depósito", value=str(int(u['deposito_base'])))
    ...
    with t4: # BORRAR CONTRATOS
        ...
        if not c_df.empty:
            id_c_del = st.selectbox("Contrato:", c_df['id'].tolist(), format_func=lambda x: c_df[c_df['id']==x]['ref'].values[0])
            if st.button("ELIMINAR CONTRATO Y DEUDAS"):
                con.execute("DELETE FROM deudas WHERE id_contrato=?", (id_c_del,))
                con.execute("DELETE FROM contratos WHERE id=?", (id_c_del,))
                con.commit(); st.rerun()
        con.close()

    with t5: # PROCESOS MASIVOS
        st.write("#### 🚀 Generación de Cuotas")
        mes_anio = st.text_input("Periodo (Ej: Abril 2026)")
        if st.button("GENERAR PARA TODOS LOS ACTIVOS"):
            ...
            con.commit(); con.close(); st.success("¡Cuotas generadas exitosamente!")
