    ...
    if not deudas.empty:
        for _, r in deudas.iterrows():
            with st.expander(f"💰 {r['nombre']} - {r['tipo']} ({r['concepto']})"):
                ...
                if c2.button("Eliminar Deuda", key=f"del_{r['id']}"):
...
# --- SECCIÓN 4: MOROSOS ---
    ...
    else: st.success("No hay deudores.")
    conn.close()
...
# --- SECCIÓN 6: MAESTROS ---
    ...
    with t1: # INQUILINOS (ABM)
        ...
        with c2:
            if not inqs_df.empty:
                ...
                if st.button("🗑️ Borrar"): con.execute("DELETE FROM inquilinos WHERE id=?", (id_del,)); con.commit(); st.rerun()
        con.close()
    ...
    with t3: # UNIDADES (ABM)
        ...
        with c1:
            ...
            with st.form("fu", clear_on_submit=True):
                ...
                tp = st.text_input("Nombre", value=v_utp)
    ...
    with t4: # CONTRATOS (VER Y BORRAR)
        ...
        st.dataframe(df_c, use_container_width=True, hide_index=True)
        if not df_c.empty:
            idc = st.selectbox("Eliminar Contrato", df_c['id'].tolist(), format_func=lambda x: f"ID {x} - {df_c[df_c['id']==x]['Inquilino'].values[0]}")
